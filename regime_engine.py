...
def calculate_regime(data: pd.DataFrame):
    ...
    if 'Close' not in data.columns or data['Close'].empty:
        raise ValueError("DataFrame must have 'Close' column with values")

    # 50-day moving average
    data['DMA50'] = data['Close'].rolling(window=50).mean()

    # Drop rows where Close or DMA50 is NaN
    data = data.dropna(subset=['Close', 'DMA50'])

    if data.empty:
        raise ValueError("No data available after calculating DMA50")
    ...
    # Volatility score (20-day rolling std)
